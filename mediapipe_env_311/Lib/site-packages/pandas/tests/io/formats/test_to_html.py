...
def test_to_html_regression_GH6098():
    df = DataFrame(
        {
            "cl1": ["a", "a", "b", "b", "a"],
            "cl2": ["1er", "2me", "1er", "2me", "1er"],
            "donnes1": np.random.default_rng(2).standard_normal(5),
            "donnes2": np.random.default_rng(2).standard_normal(5),
        }
    )

    # it works
    df.pivot_table(index=["cl1"], columns=["cl2"])._repr_html_()

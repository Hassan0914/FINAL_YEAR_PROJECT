...
class TestStata:
    ...
    def test_read_dta18(self, datapath):
        ...
        expected = DataFrame.from_records(
            [
                ["Cat", "Bogota", "Bogot", 1, 1.0, "option b nicode", 1.0],
                ["Dog", "Boston", "Uzunkpr", np.nan, np.nan, np.nan, np.nan],
                ["Plane", "Rome", "Troms", 0, 0.0, "option a", 0.0],
                ["Potato", "Tokyo", "Elz", -4, 4.0, 4, 4],  # noqa: RUF001
    ...
    def test_invalid_variable_label_encoding(self, version, mixed_frame):
        mixed_frame.index.name = "index"
        variable_labels = {"a": "very long" * 10, "b": "City Exponent", "c": "City"}
        variable_labels["a"] = "invalid character "
        with tm.ensure_clean() as path:
            with pytest.raises(
...
"""
    ...
    def test_invalid_file_not_written(self, version):
        content = "Here is one ____ Another one ____ Another one ____"
    ...
    def test_strl_latin1(self):
        ...
        output = DataFrame(
            [["pandas"] * 2, [""] * 2], columns=["var_str", "var_strl"]
        )
...
the string values returned are correct."""
        ...
        expected = DataFrame([["Dsseldorf"]] * 151, columns=["kreis1849"])
    ...
    def test_utf8_writer(self, version):
        cat = pd.Categorical(["a", "", ""], ordered=True)
        data = DataFrame(
            [
                [1.0, 1, "", " relatively long tring"],
                [2.0, 2, "", ""],
                [3.0, 3, "", None],
            ],
            columns=["", "", "", "strls"],
        ...
        data_label = "aa-label"
        value_labels = {"": {1: "label", 2: "", 3: "ot valid latin-1"}}
        ...
        with tm.ensure_clean() as path:
            ...
            data[""] = (
                data[""].replace(value_labels[""]).astype("category").cat.as_ordered()
            )

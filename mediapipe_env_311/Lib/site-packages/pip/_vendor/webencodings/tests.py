...
def test_labels():
    ...
    assert lookup('LATN1') is None  # ASCII-only case insensitivity.
...
def test_invalid_label():
    ...
    assert_raises(LookupError, encode, '', 'invalid')
...
def test_decode():
    ...
    assert decode(b'\xc3\xa9', 'utf8') == ('', lookup('utf8'))
    assert decode(b'\xc3\xa9', UTF8) == ('', lookup('utf8'))
    assert decode(b'\xc3\xa9', 'ascii') == ('', lookup('ascii'))
    assert decode(b'\xEF\xBB\xBF\xc3\xa9', 'ascii') == ('', lookup('utf8'))  # UTF-8 with BOM

    assert decode(b'\xFE\xFF\x00\xe9', 'ascii') == ('', lookup('utf-16be'))  # UTF-16-BE with BOM
    assert decode(b'\xFF\xFE\xe9\x00', 'ascii') == ('', lookup('utf-16le'))  # UTF-16-LE with BOM
    assert decode(b'\xFE\xFF\xe9\x00', 'ascii') == ('\ue900', lookup('utf-16be'))
    assert decode(b'\xFF\xFE\x00\xe9', 'ascii') == ('\ue900', lookup('utf-16le'))

    assert decode(b'\x00\xe9', 'UTF-16BE') == ('', lookup('utf-16be'))
    assert decode(b'\xe9\x00', 'UTF-16LE') == ('', lookup('utf-16le'))
    assert decode(b'\xe9\x00', 'UTF-16') == ('', lookup('utf-16le'))
...
def test_encode():
    assert encode('', 'latin1') == b'\xe9'
    assert encode('', 'utf8') == b'\xc3\xa9'
    assert encode('', 'utf8') == b'\xc3\xa9'
    assert encode('', 'utf-16') == b'\xe9\x00'
    assert encode('', 'utf-16le') == b'\xe9\x00'
    assert encode('', 'utf-16be') == b'\x00\xe9'
...
def test_iter_decode():
    ...
    assert iter_decode_to_string([b'\xe9'], 'latin1') == ''
    ...
    assert iter_decode_to_string([b'\xc3\xa9'], 'latin1') == ''
    assert iter_decode_to_string([b'\xEF\xBB\xBF\xc3\xa9'], 'latin1') == ''
    assert iter_decode_to_string([
        b'\xEF\xBB\xBF', b'\xc3', b'\xa9'], 'latin1') == ''
    assert iter_decode_to_string([
        b'\xEF\xBB\xBF', b'a', b'\xc3'], 'latin1') == 'a\uFFFD'
    assert iter_decode_to_string([
        b'', b'\xEF', b'', b'', b'\xBB\xBF\xc3', b'\xa9'], 'latin1') == ''
    assert iter_decode_to_string([b'\xEF\xBB\xBF'], 'latin1') == ''
    assert iter_decode_to_string([b'\xEF\xBB'], 'latin1') == ''
    assert iter_decode_to_string([b'\xFE\xFF\x00\xe9'], 'latin1') == ''
    assert iter_decode_to_string([b'\xFF\xFE\xe9\x00'], 'latin1') == ''
    assert iter_decode_to_string([
...
def test_iter_encode():
    ...
    assert b''.join(iter_encode([''], 'latin1')) == b''
    assert b''.join(iter_encode([''], 'latin1')) == b'\xe9'
    assert b''.join(iter_encode(['', '', '', ''], 'latin1')) == b'\xe9'
    assert b''.join(iter_encode(['', '', '', ''], 'utf-16')) == b'\xe9\x00'
    assert b''.join(iter_encode(['', '', '', ''], 'utf-16le')) == b'\xe9\x00'
    assert b''.join(iter_encode(['', '', '', ''], 'utf-16be')) == b'\x00\xe9'

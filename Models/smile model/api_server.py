...
USERS_DB = os.path.join(BASE_PATH, "users_db.json")
HISTORY_DB = os.path.join(BASE_PATH, "history_db.json")
...
def save_analysis_history(user_id: str, result: dict):
...
async def login(credentials: UserLogin):
    ...
    users = load_users()
    user = next(
        (u for u in users if u["email"].lower() == credentials.email.lower()),
        None
    )
...
async def analyze_video(
    ...
    try:
        ...
        # Save to history
        save_analysis_history(current_user["id"], result.dict())
